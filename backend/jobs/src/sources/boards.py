...
def scan_board(
    ...
    for posting in postings:
        if _keep(posting, provider, criteria, client, now):
...
def _keep(
    ...
    if not location_pending and not location_matches(posting.location, criteria.country):
        return False
    if location_pending or posting.published_at is None or not posting.description:
        _enrich(posting, provider, client)
    return location_matches(posting.location, criteria.country) and is_recent(
